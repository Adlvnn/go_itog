...
func (t Training) distance() float64 {
	return float64(t.Action) * t.LenStep / float64(MInKm)// вставьте ваш код ниже
...
type Running struct {
	Training // добавьте необходимые поля в структуру
...
func (r Running) Calories() float64 {
	...
	kolRun := ((float64(CaloriesMeanSpeedMultiplier) * r.meanSpeed()) + CaloriesMeanSpeedShift) * r.Weight / float64(MInKm) * r.Duration.Hours() * float64(MinInHours)
    return kolRun
}
...
func (w Walking) Calories() float64 {
	kolXod := (CaloriesWeightMultiplier * w.Weight + (math.Pow(w.meanSpeed() * KmHInMsec, 2) / (w.Height / 100.0)) * CaloriesSpeedHeightMultiplier * w.Weight) * w.Duration.Hours() * float64(MinInHours)
    return kolXod
}
...
func (s Swimming) Calories() float64 {
	kolPlaw := (s.meanSpeed() + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * s.Duration.Hours()  
    return kolPlaw
}
...
func (s Swimming) TrainingInfo() InfoMessage {
	return InfoMessage{
		...
		Duration:     s.Duration,
		Distance:     s.distance(), // используем шаги (гребки)
		Speed:        s.meanSpeed(),
		Calories:     s.Calories(),
	}
